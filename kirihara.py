...
#===========Hiburan============#
                          ...
                          if msg._from in admin:
                            ...
                            with requests.session() as web:
                                ...
                                if "result" not in data:
                                    ret_ = "「Status Cuaca」"
                                    ret_ += "\n♦️Lokasi : " + data[0].replace("Temperatur di kota ","")
                                    ret_ += "\n♦️Suhu : " + data[1].replace("Suhu : ","") + " C"
                                    ret_ += "\n♦️Kelembaban : " + data[2].replace("Kelembaban : ","") + " %"
                                    ret_ += "\n♦️Tekanan udara : " + data[3].replace("Tekanan udara : ","") + " HPa"
                                    ret_ += "\n♦️Kecepatan angin : " + data[4].replace("Kecepatan angin : ","") + " m/s"
                                    ret_ += "\n\nTanggal : " + datetime.strftime(timeNow,'%Y-%m-%d')
                                    ret_ += "\nJam : " + datetime.strftime(timeNow,'%H:%M:%S')
                                cl.sendMessage(msg.to, str(ret_))
                          ...
                          if msg._from in admin:
                            try:
                                ...
                                link = "♦️Link : " + "https://www.instagram.com/" + instagram
                                text = "♦️Name : "+namaIG+"\n♦️Username : "+usernameIG+"\n♦️Biography : "+bioIG+"\n♦️Follower : "+followerIG+"\n♦️Following : "+followIG+"\n♦️Post : "+mediaIG+"\n♦️Verified : "+verifIG+"\n♦️Private : "+privateIG+"" "\n" + link
                           ...
                           if msg._from in admin:
                              ...
                              if spl == 'on':
                                  if msg.to in protectantijs:
                                       ...
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "Kirihara Cover Diaktifkan\nDi Group : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「Diaktifkan」\n" + msgs)
                              elif spl == 'off':
                                    if msg.to in protectantijs:
                                         protectantijs.remove(msg.to)
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "Kirihara Cover Dinonaktifkan\nDi Group : " +str(ginfo.name)
                                    else:
